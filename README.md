...
- **Sponsor payments (Option 2 only):** `/admin/sponsors` set amount owed → **Email pay link** (`/sponsor/pay/[token]`, Stripe card). No package levels — just “sponsor” + amount. Logo publishes automatically after Stripe webhook (admin **Mark paid (manual)** only for comps). Test invite + thanks at `/admin/email`. Public `/sponsor` removed (redirects home). See `scripts/migrate-sponsor-payments.sql`
