...
- Registration email: confirmation on paid confirm (Stripe webhook + admin Mark paid) via **SendKit**; branded HTML matches site chrome (fairway-green header, warm page, logo); team ticket `/ticket/[code]` (captain enters teammate emails → personal `/ticket/p/[code]` QRs); daily cron (`vercel.json` 14:00 UTC) sends a 7-day teammate reminder; roster/check-in can **Resend confirmation**
