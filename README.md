...
- **Sponsor payments:** public levels + custom amount at `/sponsor` (Stripe card checkout; Venmo link on pay page). Admin can set amount owed → email pay link → logo publishes after Stripe webhook or **Mark paid (Venmo/check)**. See `scripts/migrate-sponsor-payments.sql` (also auto-applied via `ensureSponsorPaymentColumns`)
